	...
	class ClientChat implements Runnable{
		...
		public void end() {
			...
			try {
				...
				Cleanup.writeMessage(e, "ClientChat - end()");
			}
		}
		public void run() {
			while(connect) {
				try {
					if(initializer) {
						Socket socketOut= new Socket(ip, gamePorts[5]);
						System.out.println("Chat: Ports set");
						Socket socketIn = new Socket(ip, gamePorts[4]);
						System.out.println("Chat: Ports set");
						System.out.println("Chat: Sockets accepted");
						writer = new ObjectOutputStream(socketOut.getOutputStream());
						reader = new ObjectInputStream(socketIn.getInputStream());
						connect = false;
					}else {
						Socket socketOut= new Socket(ip, gamePorts[6]);
						System.out.println("Ports set");
						Socket socketIn = new Socket(ip, gamePorts[7]);
						System.out.println("Ports set");
			...
			while(runChat) {
				try {
					writer.writeObject("GUI.getMessage(message)");
					while(!reader.readObject().equals(prevMessage)) {
						prevMessage = reader.readObject();
						//GUI.update((ArrayList<Message> prevMessage));
					}
				}catch(Exception e) {
					Cleanup.writeMessage(e, "ClientChat - run()");
				}
			}
		}
	}*/
